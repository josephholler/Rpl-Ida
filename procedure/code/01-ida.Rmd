...
```{r}
counties <- counties %>%
  mutate(ntdi = (event_tweets - twitter_pop)/(event_tweets + twitter_pop)) %>% 
...
counties <- counties %>%
  ...
  mutate(nov = replace_na(nov, 0),
         tweetrate_nov = event_tweets / nov)

counties <- counties %>%
  mutate(ndti = (event_tweets - nov) / (event_tweets + nov)) %>% # normalized diff tweet index

...
#tdcontrol_raw <- readRDS(here("data", "raw", "private", "tdcontrol_raw.RDS")) # need different baseline
```
...
# Create a buffer around the coordinates (using sf package)
center_point <- st_point(c(center_lon, center_lat))
buffer <- st_buffer(center_point, dist = 1609.34 * 1000)  # 1609.34 meters in a mile
...
census_vars <- c("B01001_007", "B01001_008", "B01001_009")
...
county_demog <- acs_data_wide %>% 
  select(GEOID, NAME) %>% 
  mutate(male_18_29 = rowSums(acs_data_wide[3:7]),
...
# calculate number of Twitter users
# twitter_demog <- county_demog %>% 
#   mutate()
...
counties <- counties %>%
  mutate(ntdi = (event_tweets - tdcontrol) / (event_tweets + tdcontrol)) %>% 

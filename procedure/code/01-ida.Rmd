...
This study is a replication of Professor Holler's study of Twitter activity during Hurricane Dorian. 
...
**Note:** Twitter (now X) no longer offers a free API for pulling large search queries of tweets. For this study, I used tweets that were already downloaded from Twitter.
The following code will no longer work but remains in the code for transparency of the data collection methods.

## Visualize geographic region of search queries
The search queries for the Hurricane Ida tweets were limited to a geographic region of a 1000mile radius around 36,-87. 
```{r query-region}
...
search_buff <- st_buffer(center_sf, 1609000)
...
**Note:** Instead of using generic tweets to normalize the hurricane tweets, I use Census population data.
...
For the original Hurricane Dorian study, generic tweets were searched for after the hurricane
on November 19, 2019. I try using this data to normalize the Hurricane Ida tweets in addition to the Census
data to see if the results are similar (i.e. is population a good proxy for number of non-hurricane tweets?)
```{r}
...
**Note:** the `counties` dataset generated by the following code chunk was loaded for the tweet geographic region visualization.
...
counties <- filter(
  ...
  mutate(DENSITY = POP/ALAND) # calculate density, not included in ACS
...
```{r}
state_frame <- data.frame(states)

state_frame %>% arrange(states) %>% 
       mutate(states = paste(states, collapse=", "))
```
...
counties <- counties %>%
  ...
  mutate(ndti_nov = as.numeric(ndti_nov))
...
# so many 0's because NAs are being replaced with 0?
```
...
- look at extent, are hotspots in places not affected by hurricane signficant?
